...
weightsFile = "./openpose/pose_iter_160000.caffemodel"
...
while cv2.waitKey(1) < 0 and timer <= 30:  # 아무 키나 누르면 끝난다.
    # 웹캠으로부터 영상 가져옴
    ...
    dataRecord.append((round(timer, 1), points))

capture.release()  # 카메라 장치에서 받아온 메모리 해제
cv2.destroyAllWindows()  #모든 윈도우 창 닫음cv2.destroyAllWindows()

csvManager.clear_csv_file() # 데이터 리셋
csvManager.add_csv(dataRecord)